...
export async function proxyRequest(
  ...
  // Fetch active rules for the workspace (if provided)
  let rules: InterceptorRule[] = [];
  try {
    if (workspaceId) {
      // Get workspace-scoped rules
      rules = await apiClient.getActiveRulesForProxy(workspaceId);
    }
    // Note: Rules are workspace-scoped, so no fallback when workspaceId is not provided
  } catch (error) {
  ...
  try {
    ...
    for (const rule of sortedRules) {
      ...
      if (pattern.test(targetUrl)) {
        ...
        console.log(
          `[Proxy] Applying rule: ${rule.type} (${rule.id}) to ${targetUrl}`,
        );

        if (rule.type === "BLOCK") {
          return new Response(
            JSON.stringify({
              error: "Blocked by FluxPort Interceptor",
              ruleId: rule.id,
            }),
            {
              status: 403,
        ...
        if (rule.type === "DELAY" && rule.config?.delayMs) {
          await new Promise((resolve) =>
            setTimeout(resolve, rule.config.delayMs),
          );
        }
    ...
    if (appliedRuleId) {
      const modRule = sortedRules.find(
        (r) => r.id === appliedRuleId && r.type === "MODIFY_HEADERS",
      );
    ...
    try {
      ...
      if (appliedRuleId) {
        const replaceRule = sortedRules.find(
          (r) => r.id === appliedRuleId && r.type === "REPLACE_BODY",
        );
    ...
    if (errorCode === "ECONNREFUSED") {
      ...
      suggestion =
        "Ensure the service at the target URL is active and listening on the correct port.";
    } else if (errorCode === "ENOTFOUND") {
      userMessage = "Target host not found (DNS failure).";
      suggestion =
        "Check the spelling of the hostname and your internet connection.";
    } else if (
      ...
      suggestion =
        "The target server is taking too long to respond. It might be overloaded or blocked by a firewall.";
    } else if (errorMessage.toLowerCase().includes("fetch failed")) {
      userMessage = "Connection failed in the proxy layer.";
      suggestion =
        "Try unchecking 'Proxy' to connect directly from your browser. This often fixes issues with local or internal endpoints.";
    }

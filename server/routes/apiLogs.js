...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'SELECT * FROM "ApiLog" WHERE "userId" = $1 ORDER BY timestamp DESC LIMIT $2 OFFSET $3',
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    if (!requestUrl || !requestMethod || !responseStatus || latencyMs === undefined) {
      return res.status(400).json({ error: 'All fields are required' });
    }

    const result = await pool.query(
      `INSERT INTO "ApiLog" (id, "userId", "requestUrl", "requestMethod", "responseStatus", "latencyMs", timestamp) 
...
router.get('/stats', authenticateToken, async (req, res) => {
  try {
    const logsResult = await pool.query(
      'SELECT * FROM "ApiLog" WHERE "userId" = $1 ORDER BY timestamp DESC LIMIT 1000',
      [req.user.userId]
    );

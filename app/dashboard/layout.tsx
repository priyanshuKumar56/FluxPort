...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const localWorkspaces = workspaces.filter(w => !w.isTeam)
              ...
              <DropdownMenuContent align="start" className="w-[340px] p-0 border-border bg-[#222222] shadow-2xl rounded-xl overflow-hidden mt-1 text-foreground">
                ...
                <div className="p-3 pb-2 pt-4">
                  <div className="flex items-center justify-between mb-2 px-1">
                    <span className="text-sm font-medium text-foreground/80">Local workspaces ({localWorkspaces.length})</span>
                  ...
                  <div className="space-y-1">
                    {localWorkspaces.length === 0 ? (
                      <p className="text-xs italic text-muted-foreground px-2">No local workspaces.</p>
                    ) : (
                      localWorkspaces.map(ws => (
                        <div key={ws.id} onClick={(e) => { e.preventDefault(); dispatch({ type: 'workspaces/setActiveWorkspace', payload: ws.id }) }} className={cn("flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors", ws.id === activeWorkspaceId ? "bg-white/10" : "hover:bg-white/5")}>
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-foreground/50 truncate w-[200px]">C:\Users\priyanshu\Documents\Requestly\Local</span>
                            </div>
                          </div>
                    ...
                    {teamWorkspaces.map(ws => (
                      <div key={ws.id} onClick={(e) => { e.preventDefault(); dispatch({ type: 'workspaces/setActiveWorkspace', payload: ws.id }) }} className={cn("flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors", ws.id === activeWorkspaceId ? "bg-white/10" : "hover:bg-white/5")}>
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="text-sm font-medium text-white">{ws.name}</span>
                            <span className="text-xs text-foreground/50">1 member</span>
                          </div>
                        </div>
                        {ws.id === activeWorkspaceId && (
